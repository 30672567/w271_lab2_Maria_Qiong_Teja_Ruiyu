...
```{r fig5, echo=FALSE, out.width = '55%', fig.show = 'hold', fig.pos='H', fig.cap="Present CO2 and First Differenced"}
...
```{r Data pipeline and reading data, message = F, include = F}
# Website went down 
...
class(co2_present)
```
```{r EDA - dataset manipulation, include = F}
...
```
```{r EDA - plots, warning = F, include = F}
# Time plot
...
```
```{r EDA - plots diff, warning = F, include = F}
# Time plot
...
```
```{r EDA - KPSS original data, include = F}
...
print(co2_kpss)
```
```{r EDA - KPSS differencing data, include= F}
...
```
```{r EDA - classical decomposition: original and differenced data, include = F}
...
```
```{r EDA - STL decomposition, include = F}
...
```
```{r EDA - 5 year moving average growth rate, warning = F, include = F}
...
```
```{r, create monthly data, include = F}
...
```
```{r, seasonal adjustment, include = F}
...
```{r fig8, echo=FALSE, out.width = '60%', fig.show = 'hold', fig.pos='H', fig.cap="Additive and STL Decompositions"}
...
```{r, fig.width = 7, fig.height = 4, fig.align = 'center', warning = F, include = F}
...
```
```{r fig.width=10, fig.height=5, fig.align="center", warning=FALSE, include = F}
...
decomp.plot <- (p33 + p34) / (p35 + p36)
ggsave("decomp.plot.png", plot = decomp.plot, width = 7, height = 4)
```
```{r Polynomial and Seasonal Dummy Variables Model Residuals, include = F}
...
```{r original model residual, include=F}
...
```{r fig16, echo=FALSE, out.width = '50%', fig.show = 'hold', fig.pos='H', fig.cap="Linear and ARIMA Model Forecast and Plot of Seasonally Adjusted CO2 and Non-seasonally Adjusted CO2"}
knitr::include_graphics("linear.forec.comp.png")
knitr::include_graphics("arima.forec.comp.png")
```
...
*Need to choose which plot or output to place here from forecast evaluation *
...
```
```{r forecast evaluation,fig.width = 12, fig.height = 4, fig.align = 'center', warning = F, include = F}
...
hist_linear_diff <- comps2 %>%
  ...
  labs(
    title = "Linear Model CO_2 Difference Histogram",
    x = "Difference"
  ) +
  theme(legend.position = c(.2, .8))
...
hist_arima_diff <- comps2 %>%
  ...
  labs(
    title = "ARIMA Model CO_2 Difference Histogram",
    x = "Difference"
  ) +
  theme(legend.position = c(.2, .8))
...
```{r EDA - plots, monthly_mean_co2_df, non-seasonally adjust, warning = F, include= F}
# Time plot
...
```
```{r EDA - plots, monthly_mean_co2_df, seasonally adjust, warning = F, include = F}
# Time plot
...
```
```{r, difference of sa_co2, include = F}
monthly_mean_co2_df %>%
