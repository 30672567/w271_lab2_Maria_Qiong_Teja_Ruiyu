...
frequency(monthly_mean_co2$time_index)
frequency(monthly_mean_co2)
...
```{r, decomposition}
# Decomposition
co2_stl_monthly <- monthly_mean_co2  %>%
  model(STL(co2 ~ season(window = "periodic")))

co2_components_monthly <- components(co2_stl_monthly)

co2_mod_monthly.ts <- ts(monthly_mean_co2$co2, frequency = 12)
...
monthly_mean_co2$sa_co2 <- monthly_mean_co2$co2 - co2_monthly_decomp$seasonal
...
monthly_mean_co2_pivot <- monthly_mean_co2[, c("month", "year", "co2")] %>%
...
train_monthly <- subset(monthly_mean_co2, year < 2023)
test_monthly <- subset(monthly_mean_co2, year >= 2023)
...
model_fit2 <- train_monthly %>%
  model(model2 = ARIMA(sa_co2 ~ 0 + pdq(0:3,1:2,0:3) + PDQ(0:3,0:1,0:3, period=12), 
                       ic="bic", stepwise=F, greedy=F))
model_fit2 %>% report()
model_fit2 %>% coef()
```
...
##### Non-seasonally adjusted series #####
mod.quad_nsa1 <- train_monthly %>%
  model(trend_model = TSLM(co2 ~ trend() + I(trend()^2) + season())) 
mod.quad_nsa1 %>% gg_tsresiduals() + labs(title="Polynomial and Seasonal Dummy Variables Model Residuals")

mod.quad_nsa2 <- train_monthly %>%
  model(trend_model = TSLM(co2 ~ trend() + I(trend()^2) + I(trend()^3) + season())) 
mod.quad_nsa2 %>% gg_tsresiduals() + labs(title="Polynomial and Seasonal Dummy Variables Model Residuals")

##### Seasonally adjusted series #####
mod.quad_sa1 <- train_monthly %>%
  model(trend_model = TSLM(sa_co2 ~ trend() + I(trend()^2))) 
mod.quad_sa1 %>% gg_tsresiduals() + labs(title="Polynomial Model Residuals")

mod.quad_nsa2 <- train_monthly %>%
  model(trend_model = TSLM(sa_co2 ~ trend() + I(trend()^2) + I(trend()^3))) 
mod.quad_nsa2 %>% gg_tsresiduals() + labs(title="Polynomial Model Residuals")
