...
```{r}
df_test <- monthly_mean_co2 %>%
  mutate(year = year(time_index),
         month = month(time_index, label = TRUE))

# Pivot the data to wide format with years as rows and months as columns
df_test <- df_test %>%
  pivot_wider(names_from = month, values_from = co2)

# Calculate seasonal factors
plot(decompose(monthly_mean_co2))
plot(decompose(co2_present$average))

decompose(monthly_mean_co2)

# Decompose time series using STL decomposition
decomposition <- stl(monthly_mean_co2$co2, s.window = "periodic")
...
seasonal_factors <- stl(monthly_mean_co2$co2, s.window = "periodic")$time.series[, "seasonal"]

# Produce non-seasonal adjusted series
non_seasonal_adjusted_series <- your_seasonal_data - seasonal_factors
```
